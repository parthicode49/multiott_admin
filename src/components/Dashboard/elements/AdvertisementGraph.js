...
import { advertisement_view_graph } from '../../../actions/dashboard';
import { useDispatch,useSelector } from 'react-redux';
const AdvertisementGraph = () => {
  const user=useSelector((state) => state.layout.profile)
  const dispatch=useDispatch()
  useEffect(()=>{
    dispatch(advertisement_view_graph({user:user?.id}))
	},[])
  const data = useSelector((state) => state.dashboard.advertisement_view_graph);
