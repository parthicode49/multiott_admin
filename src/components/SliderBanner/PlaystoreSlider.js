...
import * as Action from "../../actions/pssliderbanner";
import Export from "../utils/Export";
import dayjs from "dayjs";
import { bindActionCreators } from "redux";
import { play_store_all_movie_list } from "../../actions/Movie/playstoremovie";

const PlaystoreSlider = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const user = useSelector((state) => state.layout.profile);
  const series = useSelector((state) => state?.webseries?.series_name);
  const rights = useSelector((state) => state.layout.rights);
  const movie = useSelector((state) => state?.movies?.ps_movies);
  const { play_store_sliderbanner_create, play_store_sliderbanner_update } = bindActionCreators(
    Action,
    dispatch
  );
  ...
  const sliderbanners = useSelector(
    (state) => state.merchandise.ps_slider_banners
  );
  const [tableData, setTableData] = useState({
    tableTitle: "Plat Store Slider Banner",
    deleteRecord: Action.play_store_sliderbanner_delete,
    updateRecord: Action.play_store_slide_banner_status_update,
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      dispatch(play_store_all_movie_list());
    }
  }, [user?.id]);
  useEffect(() => {
    if (user?.id) {
      ...
      data.append("id", user?.id);
      data.append("user", user?.id);
      dispatch(Action.play_store_all_sliderbanner_list(data));
    }
  }, [user?.id, save]);
  useMemo(() => {
  ...
  const handleSubmit = async (event) => {
    ...
    data.append("user", user?.id);
    if (isEdit) {
      const resData = await play_store_sliderbanner_update(data);
      ...
      const resData = await play_store_sliderbanner_create(data);
      if (resData?.status === 200) {
        // setForm({});
        setForm({});
        setSave(!save);
...
export default PlaystoreSlider;
