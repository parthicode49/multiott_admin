...
import { all_song_list_admin, song_name_list } from "../../actions/song";
import dayjs from "dayjs";

const SongSliderBanner = () => {
  const dispatch = useDispatch();
  const user = useSelector((state) => state.layout.profile);
  const song = useSelector((state) => state?.song?.song_name_list);
  const { song_sliderbanner_create, song_slider_banner_update } =
  ...
  const sliderbanners = useSelector(
    (state) => state.merchandise.song_slider_banners
  );
  const [tableData, setTableData] = useState({
    tableTitle: "Song Slider Banner",
    deleteRecord: Action.song_slide_banner_delete,
    updateRecord: Action.song_slide_banner_status_update,
    // deleteAccess: rights?.["Slider Banner"]?.["delete"] == "true",
    customisedStatusUpdateMessage: true,
    onDeleteText: "Are you sure want to delete Song Slider Banner?",
    onActiveText: "Are you Sure want to Activate Song Slider Banner?",
    onInactiveText: "Are you Sure want to Inactivate Song Slider Banner?",
    tableHead: [
      {
        id: "song_name",
        label: "Title",
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      data.append("id", user?.id);
      data.append("user", user?.id);
      dispatch(Action.all_song_sliderbanner_list_admin(data));
    }
  }, [user?.id, save]);
  useMemo(() => {
      ...
      temp.tableBody = sliderbanners?.data?.map((ele) => ({
        ...
        expired_on:
          new Date(ele?.expire_date) > new Date() ? (
            <p style={{ color: "var(--themeFontColor)" }}>{dayjs(ele?.expire_date).format("DD-MM-YYYY")}</p>
          ) : (
  ...
  const handleSubmit = async (event) => {
    ...
    data.append("user", user?.id);
    if (isEdit) {
      const resData = await song_slider_banner_update(data);
      ...
      const resData = await song_sliderbanner_create(data);
      if (resData?.status === 200) {
        // setForm({});
        setForm({});
        setSave(!save);
...
export default SongSliderBanner;
