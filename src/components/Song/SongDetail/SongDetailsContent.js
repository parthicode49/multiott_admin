...
import { song_detail_admin } from "../../../actions/song";
...
const SongDetailsContent = () => {
  ...
  useMemo(() => {
    if (location.state?.id) dispatch(song_detail_admin({ id: location.state?.id }));
  }, [location.state?.id]);
  const data = useSelector((state) => state.song.song_detail?.data);
  ...
  const Audio_list = useSelector((state) => state?.song?.movie?.audio_file_lis);
    ...
    <>
      ...
      >
                      ...
                      >
                        ...
                        data?.cast?.map((ele) => (
                          ...
                          >
                            ...
                            />
                            <p style={{ color: "var(--themeFontColor)" }}>{ele?.cast}</p>
                            <p style={{ color: "var(--themeFontColor)" }}>{ele?.character_name}</p>
                          </Card>
                        )) 
        ...
        {/* <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
          ...
          <>
            { role !== "Distributor" &&  <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
              {" "}
...
export default SongDetailsContent;
