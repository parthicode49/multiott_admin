...
export default function ValueAddedCustomer({ path, id }) {
  ...
  const customers = useSelector((state) => state.webseries.episode_watch_user);
  const master = useSelector((state) => state.movies);
  useEffect(() => {
    dispatch(episode_watch_user_list({ id: id }));
  }, []);
  useMemo(() => {
    if (customers?.data) {
      const temp = tableData;
        temp.tableBody = customers?.data?.map((ele)=>({...ele , name : <Link to={"/customer/detail"} state={{id : ele?.user}} style={{color : "var(--gradientColor2)"}}> {ele?.mobile_number ? ele?.mobile_number : ele?.email} </Link>}));
      setTableData({ ...temp });
    }
  }, [customers]);
  const handleClose = () => {
